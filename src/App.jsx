...
function openInNightly() {
  ...
  setTimeout(() => { window.location.href = custom }, 1200)
}
...
export default function App() {
  ...
  async function refreshBalance(address = walletAddress) {
    try { setBalance((await connection.getBalance(new PublicKey(address), 'confirmed')) / 1_000_000_000) } catch { setBalance(null) }
  }
  ...
  async function connectNightly() {
    ...
    try {
      ...
      if (!nightly) {
        setStatus('idle')
        setNotice(mobile ? 'Nightly is not injected in this mobile browser. Tap “Open in Nightly” to continue safely.' : 'Nightly was not detected. Open this page inside the Nightly browser.')
        return
      }
      ...
      setStatus('idle')
      setNotice(activeGenesisHash && activeGenesisHash === cookieGenesisHash ? 'Nightly connected to Cookie Chain.' : 'Nightly connected. Select Cookie Chain in Nightly before creating a proof.')
    } catch (error) { setStatus('idle'); setNotice(friendlyError(error)) }
  }

  async function disconnect() {
    try { await getNightly()?.features?.['standard:disconnect']?.disconnect?.() } catch {}
    setAccount(null); setNetworkReady(false); setBalance(null); setNotice('')
  }
  ...
  async function createProof() {
    ...
    try {
      ...
      const features = getNightly()?.features || {}
      const serialized = transaction.serialize({ requireAllSignatures: false, verifySignatures: false })
      const chain = account?.chains?.find((value) => value.startsWith('solana:'))

      // Nightly supports signAndSendTransaction and requires an explicit chain.
      // Prefer it on mobile/custom SVM networks because it keeps signing and broadcast inside Nightly.
      ...
      if (signAndSend && chain) {
        ...
        const signature = typeof signatureBytes === 'string'
          ? signatureBytes
          : new Uint8Array(signatureBytes).reduce((s, b) => s + String.fromCharCode(b), '')
        const base58 = signature.length ? signature : String(signatureBytes)
        // Wallet Standard signatures are byte arrays; web3.js provides the canonical base58 encoding.
        const bs58 = await import('bs58')
        const txSignature = typeof signatureBytes === 'string' ? signatureBytes : bs58.default.encode(signatureBytes)
        setStatus('confirming'); setNotice('Transaction sent. Waiting for Cookie Chain confirmation...')
        await connection.confirmTransaction({ signature: txSignature, blockhash, lastValidBlockHeight }, 'confirmed')
        await saveProof(text, txSignature)
        return
      }
  ...
  async function saveProof(text, signature) {
    ...
    setRecords(next); localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); setMessage(''); setStatus('idle'); setNotice('Proof confirmed on Cookie Chain.'); refreshBalance()
  }
  ...
  return (
    <main className="page">
      ...
      <section className="workspace">
        <div className="card composer"><div className="cardHeading"><div><span className="step">01</span><h2>Create a proof</h2></div><span className="limit">{message.length}/500</span></div>
          ...
          <button className="primary" onClick={createProof} disabled={!connected || status === 'signing' || status === 'confirming'}><span>{status === 'signing' ? 'Waiting for Nightly…' : status === 'confirming' ? 'Confirming on-chain…' : 'Create on Cookie Chain'}</span><span>→</span></button>
          {notice && <div className="notice">{notice}</div>}
        </div>
        <aside className="card side"><div className="step">WALLET</div><h2>{connected ? 'Connected' : 'Not connected'}</h2>
          {!connected ? <><button className="secondary" onClick={connectNightly}>Connect Nightly</button>{mobile && <><button className="secondary mobileNightly" onClick={openInNightly}>Open in Nightly</button><p className="mobileHint">On Android, open this app inside Nightly so the wallet can inject securely.</p></>} </> : <><div className="walletBox"><span>Address</span><strong>{shortAddress}</strong><code>{walletAddress}</code></div><div className="balance"><span>COOK balance</span><strong>{balance === null ? '—' : `${balance.toFixed(6)} COOK`}</strong></div><button className="secondary" onClick={disconnect}>Disconnect</button></>}
          <div className="chainInfo"><div><span>Network</span><strong>Cookie Chain</strong></div><div><span>RPC</span><strong>rpc.cookiescan.io</strong></div><div><span>Memo program</span><strong>Memo v1</strong></div><div><span>Network status</span><strong>{networkReady ? 'Ready' : '—'}</strong></div></div>
        </aside>
      </section>
      <section className="records"><div className="sectionTitle"><div><span className="step">02</span><h2>Your proofs</h2></div><span>{records.length} local record{records.length === 1 ? '' : 's'}</span></div>
        {records.length === 0 ? <div className="empty"><span>◌</span><h3>No proofs yet</h3><p>Connect Nightly, write a message, and create your first on-chain proof.</p></div> : <div className="recordList">{records.map((record) => <article className="record" key={record.id}><div className="recordIcon">✓</div><div className="recordBody"><p>{record.message}</p><div className="meta"><span className="status">Confirmed</span><span>{new Date(record.date).toLocaleString()}</span><span>{record.address.slice(0, 8)}…</span></div><code>{record.signature}</code></div><a className="copy" href={`${COOKIE_EXPLORER}/tx/${record.signature}`} target="_blank" rel="noreferrer">Cookiescan ↗</a></article>)}</div>}
      </section>
